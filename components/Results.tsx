...
export default function Results({
  ...
  return (
    <section style={{ maxWidth: 1100, margin: '0 auto', padding: '120px 20px 80px', position: 'relative', zIndex: 1 }}>
      ...
      {top && isUnlocked(top) && top.healthcare && (
        <motion.div
          initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.05 }}
          style={{
            background: 'rgba(90,240,200,0.04)', border: '1px solid rgba(90,240,200,0.18)',
            borderRadius: 16, padding: 24, marginBottom: 28, display: 'flex', gap: 16
          }}
        >
          <div style={{
            width: 36, height: 36, background: 'rgba(90,240,200,0.12)', borderRadius: 10,
            display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0
          }}>🏥</div>
          <div>
            <div style={{
              fontSize: 11, textTransform: 'uppercase', letterSpacing: 1.2,
              color: '#5af0c8', marginBottom: 8, fontWeight: 600
            }}>
              Healthcare for retirees
            </div>
            <p style={{ fontSize: 14, lineHeight: 1.7, color: 'rgba(240,237,232,0.85)', margin: 0 }}>
              {top.healthcare}
            </p>
          </div>
        </motion.div>
